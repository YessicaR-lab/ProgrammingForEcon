---
title: "A Cartography of Inequality: Regional Disparities in the EU Gender Pay Gap"
author: 
...
## 1.1 Description of the Social Problem
...
if (!file.exists("data/gpg_data.rds") | !file.exists("data/salary_data.rds")) {
  dir.create("data", showWarnings = FALSE)
  
  gpg_data <- get_eurostat("tesem180")
  salary_data <- get_eurostat("nama_10_fte")
  
  saveRDS(gpg_data, "data/gpg_data.rds")
  saveRDS(salary_data, "data/salary_data.rds")
}

gpg_data <- readRDS("data/gpg_data.rds")
salary_data <- readRDS("data/salary_data.rds")
```

## 2.1 A short summary of the datasets we used
...
```{r summary_salary_data, echo=FALSE}
summary(salary_data)

```

## 2.2 Description of the type of variables included in the datasets:
...
## 3.3 Visualisation of Temporal Variation
...
plot_severity <- ggplot(region_summary, aes(x = TIME_PERIOD, y = mean_gpg_severity, color = region, group = region)) +
  ...
  geom_point(size = 2) +
  labs(
    title = "1A: Average GPG Severity (EUR) \nover Time by European Region",
...
map_severity <- ggplot(europe_map, aes(x = long, y = lat, group = group)) +
  ...
  labs(
    title = paste("2A: GPG Severity in EU in", latest_year),
...
map_percent <- ggplot(europe_map, aes(x = long, y = lat, group = group)) +
  ...
  coord_quickmap(xlim = c(-25, 45), ylim = c(35, 70)) +
  labs(
    title = paste("2B: GPG % in EU in", latest_year),
...
## 3.5 Visualisation of  Sub-population Variation
...
```{r visualise_subpopulations,echo=FALSE,fig.width=6,fig.height=3,fig.align='center'}
gpg_latest %>%
...
## 3.6 Visualisation of Event analysis
...
percentage_covid_trend <- ggplot(covid_trend_summary, aes(x = TIME_PERIOD, y = mean_gpg, color = region, group = region)) +
  ...
  labs(
    ...
    subtitle = "Average percentage gap over time\nby European region, 2017-2023",
    x = "Year",
    y = "Mean GPG (%)",
    color = "Region"
  ) +
...
The gap has narrowed over the years, which is progress, but no region has actually closed it. This suggests that economic growth wages alone are not close enough to close the gap.
...
Occupational segregation likely explains a lot of this. Across Europe, men and women still cluster in different parts of the job market, men lean toward higher-paying fields like finance, tech, and engineering, while women are more often found in education, healthcare, and social work (European Institute for Gender Equality, 2023). 
...
Ultimately, the gender pay gap in Europe is a long way from being solved. Understanding what's really driving it (and what might actually reduce it) requires more research, more nuanced data, and a willingness to look beyond surface-level trends. Progress has been made, but the work is far from done.
